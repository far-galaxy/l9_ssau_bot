    ...
    def get_messages(self):
        ...
        for event in events:
            ...
            if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.from_chat:
                ...
                if event.attachments != {} and 'attachments' in event.attachments:
                    att = literal_eval(event.attachments['attachments'])
                    print(att[0]["type"])
                    if att[0]["type"] == "sticker":
                        p = {"message_ids":event.message_id, 
                             "group_id":self.group_id, 
                             "peer_id": (2000000000 + event.chat_id),
                             "delete_for_all":1}
                        self.vk.method('messages.delete', p)
                        #self.send(event.user_id, self.debug_msg)
